...
class LSM6DS33(I2C):
    ...
    def enableLSM(self, accelerometer = True, gyroscope = True,
        ...
        if accelerometer:
            ...
            self.acc_sen = 0.122   # according to table 1 for 4g
    ...
    def getGyroscope(self):
        gyroraw = self._getSensorRawLoHi3(LSM6DS33_ADDR, self.lsmAccRegisters)
        gyro = [x/29.0 for x in gyroraw]
        gyro[0]=270-gyro[0]
        # Read sensor data
        return gyro

    def getAccInG(self):
        accraw = self._getSensorRawLoHi3(LSM6DS33_ADDR, self.lsmAccRegisters)
        acc = [x*self.acc_sen/1000.0 for x in accraw]
#	acc[1]=-acc[1]
        # Read sensor data
        return acc
