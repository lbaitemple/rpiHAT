...
class RotaryEncoder:
    ...
    def start(self):
        def isr():
            self.update()
        self.gpio.wiringPiISR(self.a_pin, self.gpio.INT_EDGE_BOTH, isr)
        self.gpio.wiringPiISR(self.b_pin, self.gpio.INT_EDGE_BOTH, isr)
    ...
    class Worker(threading.Thread):
        def __init__(self, pins):
            ...
            self.stopping = False
            self.encoder = RotaryEncoder(pins)
            self.daemon = True
            self.delta = 0
            self.delay = 0.0005
            self.speed = 0
